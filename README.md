...
Finally, in order to use some of the report aliases in the sample config, e.g. 'mine' or 'to-review', add your own name to the config as follows
...
These should be written as a comma separated list of names or aliases. The available columns are
  ...
  mark                  MK      Your mark timestamp (see `marking` below)
  ...
  reviewers             R       Reviewer names; R=none matches PRs with no reviewers
                                Names are shown in green/orange/red if the reviewer has approved/commented on/rejected the PR
  ...
  valid                 V       Whether or not the PR is in a valid state, see `validity`
  ...
  workdays              WD      Total workdays logged against the YT ticket in Timely (hours/8); blank if no YT ticket
                                Note that the use of this column requires Timely admin access

Unambigious abbreviations of column names can be used, e.g. 'last' for 'last-activity'. 
...
------------
  ...
  --sort col,col,...
      - Sorts ascending by default: alphabetical for text,
        smallest-first for numbers, oldest-first for dates
      - Append :R to reverse a column's sort order
        e.g. --sort NC:R,author sorts by NC descending, then author ascending
...
- timestamp predicates
    ...
    Boolean comparisons involving timestamp columns are supported. The following are
    all valid filters

        LCT > MCT         -- true if a comment was made _after_ the user's last
        LCT > yesterday   -- true if a comment was made _after_ the start of day, yesterday
        wed < LCT         -- true if a comment was made _after_ the start of the previous wednesday
        MCT = LCT         -- true if the last comment was made at exactly the same time as
                              the user's last comment

    Note that blank timestamps, e.g. LCT when the user has _never_ made a comment, are
    treated as epoch (1970-01-01)
...
--------------------
  ...
  --include-pre-mark-commits
                       Include comments before the mark timestamp (see `marking`)
...
This allows timestamp predicates such as `LCT > MK` to be used, which could filter those PR's whose last comment was _after_ the timestamp. 
The expected use case is that a developer feels they are up to date with some PR as of some timestamp, and want to know if anything has happened to 
the PR since that point.
...
---------------
  ...
  > rtc         [ refresh the Timely event cache, see `timely reports` ]
...
The available columns are
  ...
  Rows with identical values across all displayed columns are merged and their hours summed. 
  For example the following shows one row per developer + project
...
Timely events are cached locally. The cache is refreshed automatically when a
timely report (or the workdays column) is run, but it can also be refreshed
manually with the 'rtc' command
