...
class Report:
    ...
    def aggregate(self) -> "Report":
        numeric_idx = [i for i, c in enumerate(self.cols) if c.is_numeric]
        if not numeric_idx:
            return self
        group_idx = [i for i in range(len(self.cols)) if i not in set(numeric_idx)]
    ...
    def widths(self) -> list[int]:
        hdr_lines = [c.header_lines for c in self.cols]
        ws = [max(c.display_width, max(_visible_len(l) for l in hdr_lines[i]))
              for i, c in enumerate(self.cols)]
        for row in self.rows:
            for i, val in enumerate(row):
                ws[i] = max(ws[i], _visible_len(val))
        return ws
    ...
    def render(self) -> None:
        ...
        def fmt_row(vals: list[str]) -> str:
            ...
            for i, (c, val) in enumerate(zip(cols, vals)):
                if c.is_numeric:
                    parts.append(_rjust_ansi(val, ws[i]))
                elif i == len(cols) - 1:
                    parts.append(val)
                else:
                    parts.append(_ljust_ansi(val, ws[i]))
            return " ".join(parts)
...
def _report_data_lines(
    ...
    if spec.needs_youtrack and (not config.youtrack_url or not config.youtrack_token):
        named = ", ".join("%s (%s)" % (col.name, col.aliases[0] if col.aliases else col.name)
                          for col in spec.youtrack_cols)
        raise _ListError(
            "youtrack-url and youtrack-token must be set in config to use the %s column. "
    ...
    return Report(cols=spec.cols, rows=rows)
